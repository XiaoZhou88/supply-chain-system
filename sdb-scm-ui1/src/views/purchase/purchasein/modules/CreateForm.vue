<template>
  <a-drawer width="35%" :label-col="4" :wrapper-col="14" :visible="open" @close="onClose">
    ...
    <a-form-model ref="form" :model="form" :rules="rules">
      ...
      <a-form-model-item v-show="false" label="初始票据号" prop="defaultNumber" >
      ...
      <a-form-model-item label="采购/领料-经手人id" prop="agentId" >
        <a-select v-model="form.agentId" placeholder="请输入采购/领料-经手人id">
      ...
      <a-form-model-item label="账户id" prop="accountId" >
        <a-select v-model="form.accountId" placeholder="请输入账户id">
          <a-select-option v-for="(account , index) in accountList" :key="index" :value="account.accountId">
            {{account.accountName}}
          </a-select-option>
        </a-select>
      </a-form-model-item>
      <a-form-model-item v-show="false" label="变动金额(收款/付款)" prop="changeAmount" >
      ...
      <a-form-model-item label="付款类型(现金、记账等)" prop="payType" >
        <a-select placeholder="请选择付款类型(现金、记账等)" v-model="form.payType">
          <a-select-option v-for="(d ,index) in payTypeOptions" :key="index" :value="d.dictValue" >
            {{ d.dictLabel }}
          </a-select-option>
        </a-select>
      </a-form-model-item>
      <a-form-model-item v-show="false" label="单据类型" prop="billType" >
...
<script>
import { getPurchasein, addPurchasein, updatePurchasein } from '@/api/purchase/purchasein'
import { listSupplier } from '@/api/base/supplier'
import { listAgent } from '@/api/base/agent'
import { listAccount } from '@/api/base/account'
...
export default {
  ...
  created () {
    ...
    listAccount(this.query3).then(response => {
      this.accountList = response.rows
    })
    listUser(this.query4).then(response => {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getPurchasein(jxcHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.jxcHeadId !== undefined && this.form.jxcHeadId !== null) {
            updatePurchasein(this.form).then(response => {
            ...
            addPurchasein(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
