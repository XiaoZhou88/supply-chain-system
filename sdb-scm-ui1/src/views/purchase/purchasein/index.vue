      ...
      <div class="table-page-search-wrapper">
        <a-form layout="inline">
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="出入库子类分类" prop="subType">
...
<!--              </a-col>-->
      ...
      <div class="table-operations">
        <a-button type="primary" @click="$refs.createForm.handleAdd()" v-hasPermi="['purchase:purchasein:add']">
          <a-icon type="plus" />新增
        </a-button>
        <a-button type="primary" :disabled="single" @click="$refs.createForm.handleUpdate(undefined, ids)" v-hasPermi="['purchase:purchasein:edit']">
          <a-icon type="edit" />修改
        </a-button>
        <a-button type="danger" :disabled="multiple" @click="handleDelete" v-hasPermi="['purchase:purchasein:remove']">
          <a-icon type="delete" />删除
        </a-button>
        <a-button type="primary" @click="handleExport" v-hasPermi="['purchase:purchasein:export']">
      ...
      <a-table
        ...
        <span slot="operation" slot-scope="text, record">
          <a-divider type="vertical" v-hasPermi="['purchase:purchasein:edit']" />
          <a @click="$refs.createForm.handleUpdate(record, undefined)" v-hasPermi="['purchase:purchasein:edit']">
            <a-icon type="edit" />修改
          </a>
          <a-divider type="vertical" v-hasPermi="['purchase:purchasein:remove']" />
          <a @click="handleDelete(record)" v-hasPermi="['purchase:purchasein:remove']">
            <a-icon type="delete" />删除
          </a>
        </span>
      </a-table>
...
<script>
import { listPurchasein, delPurchasein, exportPurchasein } from '@/api/purchase/purchasein'
...
export default {
  ...
  methods: {
    ...
    getList () {
      this.loading = true
      listPurchasein(this.queryParam).then(response => {
    ...
    handleDelete (row) {
      ...
      this.$confirm({
        ...
        onOk () {
          return delPurchasein(jxcHeadIds)
            .then(() => {
              that.onSelectChange([], [])
              that.getList()
    ...
    handleExport () {
      ...
      this.$confirm({
        ...
        onOk () {
          return exportPurchasein(that.queryParam)
            .then(response => {
