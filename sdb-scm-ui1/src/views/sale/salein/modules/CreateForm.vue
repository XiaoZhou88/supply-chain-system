...
<script>
import { getSalein, addSalein, updateSalein } from '@/api/sale/salein'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getSalein(jxcHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.jxcHeadId !== undefined && this.form.jxcHeadId !== null) {
            updateSalein(this.form).then(response => {
            ...
            addSalein(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
