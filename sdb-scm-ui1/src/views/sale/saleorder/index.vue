...
<!--              <a-col :md="8" :sm="24">-->
<!--                <a-form-item label="客户ID" prop="organId">-->
<!--                  <a-select v-model="queryParam.organId" placeholder="请输入客户名称">-->
<!--                    <a-select-option v-for="(customer, index) in customerList" :key="index" :value="customer.supplierName">-->
...
<!--                </a>-->
      ...
      <div class="table-operations">
        <a-button type="primary" @click="$refs.createForm.handleAdd()" v-hasPermi="['sale:saleorder:add']">
          <a-icon type="plus" />新增
        </a-button>
        <a-button type="primary" :disabled="single" @click="$refs.createForm.handleUpdate(undefined, ids)" v-hasPermi="['sale:saleorder:edit']">
          <a-icon type="edit" />修改
        </a-button>
        <a-button type="danger" :disabled="multiple" @click="handleDelete" v-hasPermi="['sale:saleorder:remove']">
          <a-icon type="delete" />删除
        </a-button>
        <a-button type="primary" @click="handleExport" v-hasPermi="['sale:saleorder:export']">
      ...
      <a-table
        ...
        <span slot="operation" slot-scope="text, record">
          <a-divider type="vertical" v-hasPermi="['sale:saleorder:edit']" />
          <a @click="$refs.createForm.handleUpdate(record, undefined)" v-hasPermi="['sale:saleorder:edit']">
            <a-icon type="edit" />修改
          </a>
          <a-divider type="vertical" v-hasPermi="['sale:saleorder:remove']" />
          <a @click="handleDelete(record)" v-hasPermi="['sale:saleorder:remove']">
            <a-icon type="delete" />删除
          </a>
        </span>
      </a-table>
...
<script>
import { listSaleorder, delSaleorder, exportSaleorder } from '@/api/sale/saleorder'
import { listCustomer } from '@/api/base/supplier'
...
export default {
      ...
      columns: [
        {
          ...
          dataIndex: 'organId',
          ellipsis: true,
          scopedSlots: { customRender: 'organId' },
          align: 'center'
        },
        // {
        //   title: '主键',
  ...
  methods: {
    ...
    getList () {
      this.loading = true
      listSaleorder(this.queryParam).then(response => {
    ...
    getCustomerName(userId){
        getCustomer(userId).then( response =>{
          this.customerName =response.data.supplierName
        })
    ...
    handleDelete (row) {
      ...
      this.$confirm({
        ...
        onOk () {
          return delSaleorder(jxcHeadIds)
            .then(() => {
              that.onSelectChange([], [])
              that.getList()
    ...
    handleExport () {
      ...
      this.$confirm({
        ...
        onOk () {
          return exportSaleorder(that.queryParam)
            .then(response => {
