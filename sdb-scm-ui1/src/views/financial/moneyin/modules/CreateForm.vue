<template>
  <a-drawer width="35%" :label-col="4" :wrapper-col="14" :visible="open" @close="onClose">
    ...
    <a-form-model ref="form" :model="form" :rules="rules">
      <a-form-model-item label="类型(支出/收入/收款/付款/转账)" prop="type" >
        <a-select placeholder="请选择类型(支出/收入/收款/付款/转账)" v-model="form.type">
          <a-select-option v-for="(d, index) in typeOptions" :key="index" :value="d.dictValue" >{{ d.dictLabel }}</a-select-option>
        </a-select>
      </a-form-model-item>
      <a-form-model-item label="单位Id(收款/付款单位)" prop="organId" >
      ...
      <a-form-model-item label="变动金额(优惠/收款/付款/实付)" prop="changeAmount" >
      ...
      <a-form-model-item label="账户(收款/付款)" prop="accountId" >
...
<script>
import { getMoneyin, addMoneyin, updateMoneyin } from '@/api/financial/moneyin'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getMoneyin(accountHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.accountHeadId !== undefined && this.form.accountHeadId !== null) {
            updateMoneyin(this.form).then(response => {
            ...
            addMoneyin(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
