...
<script>
import { getRetailin, addRetailin, updateRetailin } from '@/api/retail/retailin'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getRetailin(jxcHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.jxcHeadId !== undefined && this.form.jxcHeadId !== null) {
            updateRetailin(this.form).then(response => {
            ...
            addRetailin(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
