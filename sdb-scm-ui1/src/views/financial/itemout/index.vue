  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      <div class="table-operations">
        <a-button type="primary" @click="$refs.createForm.handleAdd()" v-hasPermi="['financial:itemout:add']">
          <a-icon type="plus" />新增
        </a-button>
        <a-button type="primary" :disabled="single" @click="$refs.createForm.handleUpdate(undefined, ids)" v-hasPermi="['financial:itemout:edit']">
          <a-icon type="edit" />修改
        </a-button>
        <a-button type="danger" :disabled="multiple" @click="handleDelete" v-hasPermi="['financial:itemout:remove']">
          <a-icon type="delete" />删除
        </a-button>
        <a-button type="primary" @click="handleExport" v-hasPermi="['financial:itemout:export']">
      ...
      <a-table
        ...
        <span slot="operation" slot-scope="text, record">
          <a-divider type="vertical" v-hasPermi="['financial:itemout:edit']" />
          <a @click="$refs.createForm.handleUpdate(record, undefined)" v-hasPermi="['financial:itemout:edit']">
            <a-icon type="edit" />修改
          </a>
          <a-divider type="vertical" v-hasPermi="['financial:itemout:remove']" />
          <a @click="handleDelete(record)" v-hasPermi="['financial:itemout:remove']">
            <a-icon type="delete" />删除
          </a>
        </span>
      </a-table>
...
<script>
import { listItemout, delItemout, exportItemout } from '@/api/financial/itemout'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      listItemout(this.queryParam).then(response => {
    ...
    handleDelete (row) {
      ...
      this.$confirm({
        ...
        onOk () {
          return delItemout(accountHeadIds)
            .then(() => {
              that.onSelectChange([], [])
              that.getList()
    ...
    handleExport () {
      ...
      this.$confirm({
        ...
        onOk () {
          return exportItemout(that.queryParam)
            .then(response => {
