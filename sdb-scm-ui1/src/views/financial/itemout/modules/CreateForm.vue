...
<script>
import { getItemout, addItemout, updateItemout } from '@/api/financial/itemout'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getItemout(accountHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.accountHeadId !== undefined && this.form.accountHeadId !== null) {
            updateItemout(this.form).then(response => {
            ...
            addItemout(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
