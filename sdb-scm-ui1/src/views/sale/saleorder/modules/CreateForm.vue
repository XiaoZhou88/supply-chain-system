...
<!--      </a-form-model-item>-->
      ...
      <a-form-model-item label="客户id" prop="organId" >
        <a-select v-model="form.organId" placeholder="请输入客户id">
          <a-select-option v-for="(customer, index) in customerList" :key="index" :value="customer.supplierId">
...
<script>
import { getSaleorder, addSaleorder, updateSaleorder } from '@/api/sale/saleorder'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getSaleorder(jxcHeadId).then(response => {
