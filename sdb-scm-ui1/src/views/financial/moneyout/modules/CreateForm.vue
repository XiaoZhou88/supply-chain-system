...
<script>
import { getMoneyout, addMoneyout, updateMoneyout } from '@/api/financial/moneyout'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getMoneyout(accountHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.accountHeadId !== undefined && this.form.accountHeadId !== null) {
            updateMoneyout(this.form).then(response => {
            ...
            addMoneyout(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
