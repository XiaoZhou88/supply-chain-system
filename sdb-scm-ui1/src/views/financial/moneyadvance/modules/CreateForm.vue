...
<script>
import { getMoneyadvance, addMoneyadvance, updateMoneyadvance } from '@/api/financial/moneyadvance'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      ...
      getMoneyadvance(accountHeadId).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.accountHeadId !== undefined && this.form.accountHeadId !== null) {
            updateMoneyadvance(this.form).then(response => {
            ...
            addMoneyadvance(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
